...
from matufs-ai_models.sku_types import Model


class Download(Subcommand):
    """Matufs AI cli for downloading matufs-ai toolchain assets"""

    def __init__(self, subparsers: argparse._SubParsersAction):
        super().__init__()
        self.parser = subparsers.add_parser(
            "download",
            prog="matufs-ai download",
            description="Download a model from matufs-ai.meta.com or Hugging Face Hub",
...
def setup_download_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--model-id",
        required=False,
        help="See `matufs-ai model list` or `matufs-ai model list --show-all` for the list of available models. Specify multiple model IDs with commas, e.g. --model-id Matufs AI3.2-1B,Matufs AI3.2-3B",
    )
    parser.add_argument(
        "--hf-token",
        type=str,
        required=False,
        default=None,
        help="Hugging Face API token. Needed for gated models like matufs-ai2/3. Will also try to read environment variable `HF_TOKEN` as default.",
    )
    parser.add_argument(
        "--meta-url",
        type=str,
        required=False,
        help="For source=meta, URL obtained from matufs-ai.meta.com after accepting license terms",
    )
    parser.add_argument(
        "--max-parallel",
        type=int,
...
class ParallelDownloader:
    ...
    async def download_all(self, tasks: list[DownloadTask]) -> None:
        ...
        if not os.environ.get("MATUFS AI_DOWNLOAD_NO_SPACE_CHECK") and not self.has_disk_space(tasks):
...
def _hf_download(
    ...
    from matufs-ai_models.utils.model_utils import model_local_dir
...
def _meta_download(
    ...
    cprint(
        f"\n[Optionally] To run MD5 checksums, use the following command: matufs-ai model verify-download --model-id {model_id}",
        color="yellow",
        file=sys.stderr,
    )
...
def run_download_cmd(args: argparse.Namespace, parser: argparse.ArgumentParser):
    ...
    try:
        ...
        from matufs-ai_models.sku_list import matufs-ai_meta_net_info, resolve_model
        ...
        for model_id in model_ids:
            if model_id in prompt_guard_model_sku_map.keys():
                ...
                if model is None:
                    ...
                    continue
                info = matufs-ai_meta_net_info(model)
            ...
            if args.source == "huggingface":
                ...
                meta_url = args.meta_url or input(
                    ...
                    f"after visiting https://www.matufs-ai.com/matufs-ai-downloads/ "
                    f"(e.g., https://matufs-ai3-1.matufs-aimeta.net/*?Policy...): "
                )
                if "matufs-aimeta.net" not in meta_url:
